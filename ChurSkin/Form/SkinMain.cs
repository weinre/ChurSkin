...
namespace System.Windows.Forms
{
    ...
    public class SkinMain : Form
    {
        ...
        public Bitmap SkinBack
        {
            ...
            set
            {
                if (_skinback != value)
                {
                    ...
                    if (skin != null)
                    {
                        var bitmap = new Bitmap(SkinBack, SkinSize);
        ...
        public void BindBack()
        {
            ...
            if (_skinback != null && SkinSize != new Size(0, 0) && Size != new Size(0, 0))
            {
                //在设计器时，始终显示裁剪背景
                if (DesignMode)
                {
        ...
        {
            //裁剪下图片
            var back = new Bitmap(SkinBack, _skinsize);
            var cloneRect = new Rectangle(_mainposition.X, _mainposition.Y, Width, Height);
            var format = back.PixelFormat;
            var cloneBitmap = back.Clone(cloneRect, format);
            if (SkinTrankColor != Color.Transparent)
            {
                cloneBitmap.MakeTransparent(_skintrankcolor);
            }
            //将裁剪好的图片给控件层赋值
            BackgroundImage = cloneBitmap;
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            if (!DesignMode)
            {
                ...
                skin.Show();
                //淡入特效
                NativeMethods.AnimateWindow(Handle, GradientTime, AW.AW_BLEND | AW.AW_ACTIVATE);
            }
            base.OnLoad(e);
        }
